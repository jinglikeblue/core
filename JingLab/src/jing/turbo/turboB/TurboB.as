package jing.turbo.turboB
{
	...
	import jing.turbo.turboB.elements.ElementB;
	
	use namespace TurboNS;
	/**
	 * 引擎核心显示类，所有基于IElement的类都是该类的子元素 
	 * 混合引擎
	 * @author jing
	 * 
	 */	
	public class TurboB implements ITurbo
	{
		...
		public function TurboB(buildBox:Sprite = null)
		{
			if(null == buildBox)
			{
				buildBox = new Sprite();
		...
		public function addElement(ele:IElement):void
		{
			...
			_buildBox.addChild((ele as ElementB).sprite);
			_eleList.push(ele);
			(ele as ElementB).setTurbo(this);
		}
		...
		public function removeElement(ele:IElement):void
		{
			...
			if(-1 != index)
			{
				(ele as ElementB).setTurbo(null);
				_buildBox.removeChild((_eleList[index] as ElementB).sprite);
		...
		public function removeElementAt(index:int):void
		{
			...
			{
				(ele as ElementB).setTurbo(null);
				_buildBox.removeChild((ele as ElementB).sprite);
				_eleList.splice(index,1);
			}
		}
		...
		public function addElementAt(ele:IElement, childIndex:int):void
		{
			...
			_buildBox.addChildAt((ele as ElementB).sprite,childIndex);
		}
